...
#Una vez encontrados los retornos logarítmicos armamos las matrices de retornos necesarias, cada una de acuerdo a la longitud de la serie de tiempo y de su tipo
...
#Establecemos el problema de optimización, en este caso con restricción de operaciones en corto y no más del 60% en un activo
pesoptim <- function(R){
  funds <- colnames(R)
  pfolio <- portfolio.spec(assets=funds)
  pfolio <- add.constraint(portfolio = pfolio, type = "full_investment")
  pfolio <- add.constraint(portfolio = pfolio, type = "box", min = 0, max=0.6)
